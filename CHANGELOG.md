...
- `llmdialect/ir.Usage.CacheReadInputTokens` and `CacheWriteInputTokens`
  ...
  a write count, since neither wire has one; the OpenAI-shaped
  ...
  `bridge.Usage` is unchanged, a floored total for a meter. What to do
  about it: a consumer that read the two fields dereferences them after
  a nil check, nil meaning unknown; one that built an `ir.Usage` literal
  takes the address of the count; and a `==` between two `ir.Usage`
  values now compares pointers, so compare members or use
  `reflect.DeepEqual`.
