...
| [`authkit`](authkit/) | The auth tree. The root holds the one `Identity` type, the `Authenticator` interface, the grants a personal access token carries, and the middleware every service shares; `authkit/jwt` verifies RS256 and ES256 tokens against a cached JWKS, authenticates bearer JWTs, warms every configured issuer's key set at start-up with `Validator.Warm` so the first request pays for no fetch, and reads a token's grants once the service sets `Config.ReadsGrants` — until it does, a scoped token is refused with reason `grants_unread` rather than admitted unnarrowed; `authkit/oidc` is the OIDC relying party with a `Provider` for any standard issuer (Latere auth, Keycloak, Google, Cognito) and the Latere `Client` with encrypted cookie sessions, token refresh, org switching, and a shared `/me` assembly; `authkit/cli` holds the token store and device-code login for command-line clients |
| [`authz`](authz/) | The one authorizer contract the open cores share: the request and decision envelope, the client with the cache and failure rules, the owner policy a self-hosted core falls back to, and a core's action table as data. A decision is narrowed by the grants the caller's own credential carries, so a key granted read on one repository is refused everything else with reason `grant`. `authz/server` is the endpoint scaffold that applies the narrowing by construction, `authz/stub` the stub authorizer for a test tier, and `authz/conformance` the suite every authorizer passes |
